...
class NRCIntegrator(mi.SamplingIntegrator):
    ...
    writer = None
        ...
        self.optimizer = torch.optim.Adam(params=nrc_net.train_mlp.parameters(), lr=nrc_config.optim_lr)
        self.writer = writer
    ...
    def sample(self, scene, sampler, ray, medium=None,
        ...
        # print('=============')
        # print(f'Starting frame {self.nrc_net.num_rendered_frame} rendering')
        ...
        for depth_i in range(self.render_config.long_depth):
            path_type = 'short' if depth_i < self.render_config.short_depth else 'long'
            
            if depth_i == self.render_config.short_depth:
                # first iter for long training path
            ...
            active_next = active_next & si.is_valid()
            ...
            if depth_i == 0:
                ...
                if depth_i == self.render_config.short_depth:
                    # reset a_sum to acculate a(xn~xm)
                    a_sum = 0
                a_curr = vertex_spread_dr(x_i_1=prev_si.p, x_i=si.p, pdf_bsdf=bsdf_sample.pdf, wi=si.to_world(bsdf_sample.wo), n=si.n)
                a_sum = a_sum + a_curr
                a_subpath = (a_sum)**2
                ...
                print(f'Ratio terminated paths: {dr.count(terminated_paths & active_next)/dr.count(active_next)}')
                ...
                if depth_i == self.render_config.short_depth-1:
            ...
            if self.nrc_config.reflectance_factorisation:
                specular_reflectance = (bsdf_weight+rew_emitter_rad).numpy()
            ...
            # NRC # 4 # NRC query: for all the terminated rays, query the neural network.re
            if self.nrc_net.num_rendered_frame > 0 and \
                depth_i != 0 and depth_i != self.render_config.short_depth:
                ...
                inference_results = np.zeros_like(diffuse_reflectance)
                inference_results[active_NRC_paths] = self.nrc_net.inference_np(
                        si.p.numpy()[active_NRC_paths],
                        si.wi.numpy()[active_NRC_paths],
                        si.n.numpy()[active_NRC_paths],
                        diffuse_reflectance[active_NRC_paths], 
                        specular_reflectance[active_NRC_paths], 
                        roughness[active_NRC_paths])

                inference_results = np.clip(inference_results, 0, None)
                filtered_nrc_results = Color3f(inference_results)
                if self.nrc_config.activate_nrc:
                    L = L + throughput*filtered_nrc_results

                # Update path activation arrays after querying NRC
                active_next = active_next & (~terminated_paths)
            elif depth_i == 0:
            ...
            # ------------------ NRC  training ----------------------------
            if depth_i == 0 or depth_i == self.render_config.short_depth or self.nrc_net.num_rendered_frame == 0:
            ...
            ## Full epoch short path training for first frame
            if self.nrc_config.first_frame_training \
                and self.nrc_net.num_rendered_frame == 0\
                and depth_i == self.render_config.short_depth-1:
                    nrc_inputs, nrc_outputs = bounce_info.gather_training_data()

                    loss = self.nrc_net.nrc_train(nrc_inputs, nrc_outputs, 
                        self.optimizer, batch_num=self.nrc_config.steps_per_frame)
                    print(f'full epoch training loss: {loss}')

            # regular training for long path
            if depth_i == self.render_config.long_depth -1:
                ...
                loss = self.nrc_net.nrc_train(nrc_inputs, nrc_outputs, 
                    self.optimizer, batch_num=self.nrc_config.steps_per_frame)
                print(f'Long path shape {nrc_outputs.shape} training Loss: {loss}')

                self.writer.add_scalar('Loss/Long Path Train Loss', loss, self.nrc_net.num_rendered_frame)
